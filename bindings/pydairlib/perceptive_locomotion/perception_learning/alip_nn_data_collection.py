...
def run(sim_env, controller, diagram):
    ...
    for i in range(1, 500): # 2000
        ...
        #        hmap[-1], x - xd, controller.ue_grid
        #    ).to(controller.device)
        #    combined_input = combined_input.unsqueeze(0)

        #    residual_grid = controller.residual_unet(combined_input).squeeze(0)
        #    residual_grid = residual_grid.detach().cpu().numpy().reshape(H, W)
        #linear_term_grid = np.einsum('n,nhw->hw', (x - xd), controller.linear_coeff_grid)
        #final_grid = controller.u_cost_grid + controller.u_next_value_grid + linear_term_grid + residual_grid + collision_cost
        ALIPtmp.append(alip)
        FOOTSTEPtmp.append(footstep)
        HMAPtmp.append(hmap)
        #COST_GRIDtmp.append(final_grid)
    return ALIPtmp, FOOTSTEPtmp, HMAPtmp, Terminate#, COST_GRIDtmp
...
def main():
    ...
    #    perception_learning_base_folder, 'params/wavy_test.yaml'#'params/stair_curriculum.yaml' #'params/wavy_terrain.yaml' # 'params/flat.yaml'
    #)
