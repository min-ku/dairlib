...
def tile_and_concatenate_inputs(heightmap, state, input_space):
    ...
    hmap_tensor = torch.tensor(hmap, dtype=torch.float32) # 1
    state_tensor = torch.tensor(state_tiled, dtype=torch.float32) # 4
    input_space_tensor = torch.tensor(input_space, dtype=torch.float32) # 2
