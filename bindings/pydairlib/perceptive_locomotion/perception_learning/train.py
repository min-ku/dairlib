...
def _run_training(config, args):
    ...
    if args.test:
        ...
        # Load model
        #model_path = 'PPO_student_rand.zip'
        #test_folder = "rl/tmp/DrakeCassie/eval_logs/test/imitation"
        #model_path = path.join(test_folder, 'best_model.zip')
        
        model = PPO(
            policy_type, env, learning_rate = linear_schedule(0.0001), n_steps=int(2048/num_env), n_epochs=10,
        ...
        #model = model.load(model_path, env, learning_rate = linear_schedule(0.0001), n_steps=int(2048/num_env), n_epochs=10, batch_size=64*num_env, tensorboard_log=tensorboard_log,)
