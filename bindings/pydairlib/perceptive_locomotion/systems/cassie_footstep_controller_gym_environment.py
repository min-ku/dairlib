...
class RewardSystem(LeafSystem):
    ...
    def calc_reward(self, context: Context, output) -> None:
        ...
        #if left_angle > 0.3:
        #    left_angle_reward = -left_angle
            #print(left_angle_reward)
        #if right_angle > 0.3:
        #    right_angle_reward = -right_angle
            #print(right_angle_reward)
        
        #wf = plant.world_frame()
        #left_toe_bf = plant.GetBodyByName("toe_left").body_frame()
        #print(left_toe_bf)
        #T = left_toe_bf.CalcPoseInWorld(plant_context)
        #R = T.GetAsMatrix4()[:3, :3]
        #cos_angle = 0.5 * (np.trace(R) - 1)
        #angle = np.arccos(np.clip(cos_angle, -1, 1))
        #right_toe_bf = plant.GetBodyByName("toe_right").body_frame()
        
        #print(angular_reward)
